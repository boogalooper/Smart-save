...
// динамическая панель для каталогов
function pnFolderAdd (parent)
{
      // GRFOLDER
    // ========
    ...
    //    for (var i=0; i<parent.children.length; i++) {if (parent.children[i] == grfolder) {parent.children[i].index =i; break}}
    // GRBTN
    // =====
 ...
 //$.writeln ('add folder group: index ' + grfolder.index)
 ...
 dlMode.onChange = function ()
 {
     ...
     case 0: break;
     case 1: grSizeAdd (grfolder); break;
     }
  }
 ...
 // группа с опциями размера
 function grSizeAdd (parent)
 {
    // GROPT
    // =====
  var gropt = parent.add("group"); 
    ...
    //
    dropdown1.onChange = function () {var tmp =putLocalSet(gropt); if (tmp!="") parent.label=tmp; collectSubfolderSet (parent.parent)}
    dropdown3.onChange = function () {var tmp =putLocalSet(gropt); if (tmp!="") parent.label=tmp; collectSubfolderSet (parent.parent)}
    ...
    dropdown2.onChange = function ()
    {
        ...
        var tmp =putLocalSet(gropt); if (tmp!="") parent.label=tmp; collectSubfolderSet (parent.parent)
     }
 
   getLocalSet ( "0|0|0", gropt)
...
function getLocalSet (s, parent)
{
   var tmp = s.split("|")
    ...
    for (var i=0; i<parent.children.length;i++)
    {
        ...
        {
            case "dropdownlist":
                parent.children[i].selection = Number(tmp[i])
                break;
            default:
...
function putLocalSet (parent)
{
 ...
 s = s.substr (0, s.length-1)} catch (e) {s=""}
 return s
 }
...
function collectSubfolderSet (parent)
{
    ...
    for (var i=0; i<parent.children.length; i++)
    {
        tmp.push (String (parent.children[i].children[1].selection.index) + '|' + parent.children[i].label)
        $.writeln ('folder settings: ' + String (parent.children[i].children[1].selection.index) + '|' + parent.children[i].label)
    }
}